...
public class IngressoController {
    ...
    public Flux<IngressoDto> totalDisponivel(@PathVariable Long id) {
        return servico.obterPorId(id)
                .flatMapMany(ingresso -> 
                    servico.obterLotesPorEvento(ingresso.eventoId())
                            .mergeWith(ingressoSink.asFlux()
                                    .filter(i -> i.eventoId().equals(ingresso.eventoId())))
                );
    }
    ...
    @PostMapping("/lotes")
    public Mono<IngressoDto> criarLote(@RequestBody NovoLoteDto dto) {
        return servico.criarLote(dto);
    }

    @GetMapping("/eventos/{eventoId}/lotes")
    public Flux<IngressoDto> listarLotesPorEvento(@PathVariable Long eventoId) {
        return servico.obterLotesPorEvento(eventoId);
    }
}
